import { renderWiki, initWiki } from './components/wiki.js';
...
const views = {
  course: {
    title: 'Staff Handbook Wiki',
    subtitle: 'Search, create, and link camp operational guidelines.',
    render: renderWiki,
    init: initWiki
  },
...
// Reduced Stimulation Toggle Engine
function initStimulation() {
  const isCalm = localStorage.getItem('camp_lawton_calm_mode') === 'true';
  if (isCalm) {
    document.documentElement.classList.add('reduced-stimulation');
    const toggle = document.getElementById('stimulation-toggle');
    if (toggle) toggle.classList.add('active');
  }
}

function toggleStimulation() {
  const isCalm = document.documentElement.classList.toggle('reduced-stimulation');
  localStorage.setItem('camp_lawton_calm_mode', isCalm ? 'true' : 'false');
  const toggle = document.getElementById('stimulation-toggle');
  if (toggle) {
    if (isCalm) toggle.classList.add('active');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const stimulationToggle = document.getElementById('stimulation-toggle');
  if (stimulationToggle) {
    stimulationToggle.addEventListener('click', toggleStimulation);
  }
    ...
    searchInput.addEventListener('input', () => {
      ...
      searchDropdown.querySelectorAll('.search-result-item').forEach(el => {
        el.addEventListener('click', () => {
          ...
          if (viewId === 'course' && match) {
            const targetSlug = match.slug || (match.title ? generateSlug(match.title) : 'welcome');
            state.wikiSlug = targetSlug;
          }
